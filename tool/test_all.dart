...
Future testHttp() async {
  ...
  await runCmd(dartanalyzerCmd(['--fatal-warnings', 'lib', 'test'])..workingDirectory = dir);
...
Future testHttpIo() async {
  ...
  await runCmd(pubCmd(pubGetArgs())..workingDirectory = dir);
  await runCmd(dartanalyzerCmd(['--fatal-warnings', 'lib', 'test'])..workingDirectory = dir);
  await runCmd(pubCmd(pubRunTestArgs(platforms: ['vm']))
    ..workingDirectory = dir);
}
...
Future testHttpRedirect() async {
  ...
  await runCmd(dartanalyzerCmd(['--fatal-warnings', 'lib'])..workingDirectory = dir);
  await runCmd(
      pubCmd(pubRunTestArgs(platforms: [/* not yet 'node' */'vm', 'chrome']))..workingDirectory = dir);
}
